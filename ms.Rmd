...
```{r setup, echo=FALSE}
# csl: /Users/sjp/MEGA/totransfer/resources/zotero_styles/apa.csl
...
```{r environment}
# install.packages(c("ggthemes", "GGally", "survminer", "maps", "maptools", "rgdal", "mapdata", "mapproj", "sm", "scales", "tidyverse", "forestplot", "gridExtra", "prevalence", "hexbin", "spatstat", "RCurl", "coxme", "nlme", "glm2", "bbmle", "emdbook", "visreg", "grid", "citr"))
# library(citr); 
library(knitr)
library(plyr); library(tidyverse); library(forcats); library(lubridate); library(stringr); library(scales)
library(ggplot2); library(GGally); library(hexbin); library(grid); library(gridExtra); library(ggthemes); library(visreg); library(ggsignif)
library(zoo); library(nlme); library(glm2); library(mgcv); library(bbmle); library(car); library(emdbook); library(modelr); library(prevalence); library(MASS); library(lme4); library(glmm)
library(maps); library(maptools); library(spatstat); library(rgdal); library(mapdata); library(mapproj); library(raster)
library(survival); library(coxme); library(survminer); library(forestplot)
library(RCurl)

load("DataS1.RData")
# Contains:
  # box_temp - in vivo challenge experiment: summary of climate chamber temperatures
  # d_fv3_invitro - FV3 growth at a range of temperatures dataset
  # dbox_temp - in vivo challenge experiment: raw data from iButton dataloggers used within climate chambers
  # dfmp - Frog Mortality Project dataset
  # dplos_csv - wild disease outbreaks caused by FV3 (from Price et al. 2017) with variables describing temperature data in the week preceding incidents (UKCP09 max. daily temperature on 5x5km grid)
  # droom_temp - in vivo challenge experiment: room temperatures
  # dsurv - in vivo challenge experiment: suvival and viral load data
  # highEmissions_cols - colours generated from probabilities of monthly av. daily max temperatures exceeding 16°C on the UK 25 x 25km grid (using climate projections for the UK for 2070-2099 under IPCC emissions scenario A1FI): orange if probability >=50%, tourquoise otherwise.
  # hist_cols - list of 12 matrices (one for each month) containing colours generated from the average daily maximum temperature across the month for the period 1991-2010 on the 5 x 5km grid of the UK: orange if it exceeds 16°C, tourquoise otherwise
  # lowEmissions_cols - colours generated from probabilities of monthly av. daily max temperatures exceeding 16°C on the UK 25 x 25km grid (using climate projections for the UK for 2070-2099 under IPCC emissions scenario B1): orange if probability >=50%, tourquoise otherwise.
  # medEmissions_cols - colours generated from probabilities of monthly av. daily max temperatures exceeding 16°C on the UK 25 x 25km grid (using climate projections for the UK for 2070-2099 under IPCC emissions scenario A1B): orange if probability >=50%, tourquoise otherwise.
  # dsigns - signs of disease data for in vivo dose experiment
  # dtiming - timing of signs of disease from in vivo dose experiment
  # dloads - viral loads from in vivo dose experiment

source("R/functions.R")
source("R/params.R")
source("R/fig_panels.R")
```
...
The finalized FMP dataset used in this study contained `r nrow(dfmp)` unique records, of which `r dfmp %>% filter(rv == 1) %>% nrow()` were classed as ranavirosis-consistent. The logistic model (Model 1) revealed a highly significant, non-linear effect of temperature on the proportion of ranavirosis-consistent incidents observed (Table S1). The minimal adequate model also retained newts, fish and shading: the presence of either type of animal in ponds increased the proportion of ranavirosis-consistent incidents observed whilst shading reduced this proportion (Table S1). To explore the relationship with temperature further, a model with a transition between an upper and lower frequency was fitted (Model 2), which significantly improved the fit to the data compared to a simplified version of Model 1 comprising only the terms describing the non-linear relationship with temperature (AIC scores were `r round(AIC(optRes))` and `r round(AIC(m_incidence_maxt2))` respectively). Model 2 shows a step-change: below approximately 16°C, `r signif(100*aFitted, digits = 3) %>% as.vector()`% of incidents were ranavirosis-consistent, rising to `r signif(100*bFitted, digits = 3) %>% as.vector()`% after the temperature threshold was crossed (Figure 1a). The difference between incidents that were ranavirosis-consistent and the remainder (‘non-ranavirus’) is also apparent in the distribution of temperature records: the non-ranavirus category being strongly bimodal with peaks at both low and high temperature, whereas most of the ranavirosis-consistent incidents were reported at higher temperatures with few outliers at much lower temperatures (Figure S3).  

```{r}
source("R/pm_db.R")
```

Temperature was again a highly significant predictor of ranavirus status when the records with precise timestamps and confirmed ranavirus-positive status were analyzed. This more precise information about timing enabled a fine-scale examination of the effect of temperature in the days preceding incidents. The average temperature in the seven days preceding incidents was a significant predictor of ranavirus status (p = $`r signif(summary(m_precise_avtemp7)$coefficients[2,4], digits = 3)`$; residual deviance of model = `r signif(m_precise_avtemp7$deviance, digits = 4)` on `r m_precise_avtemp7$df.residual` degrees of freedom; Figure 1b), with each 1°C increase in temperature increasing the odds that incidents were caused by ranavirus by `r signif(100*(exp(coefficients(m_precise_avtemp7)[2])-1) %>% as.numeric(), digits = 2)`%. The temperature threshold where the proportion of ranavirus incidents increased sharply in the analysis of the full FMP dataset was approximately 16°C. A second model - with the number of consecutive days where the daily maximum temperature in the week preceding incidents exceeded 16°C as a predictor - also indicated that warmer temperatures were a good predictor of ranavirus status (p = $`r signif(summary(m_precise_n16wk)$coefficients[2,4], digits = 3)`$; residual deviance of model = `r signif(m_precise_n16wk$deviance, digits = 4)` on `r m_precise_n16wk$df.residual` degrees of freedom; Figure 1c): each additional warm day raised the odds that incidents were caused by ranavirus by `r signif(100*(exp(coefficients(m_precise_n16wk)[2])-1) %>% as.numeric(), digits = 2)`%. The 16°C threshold model had a slightly lower AIC score than the model using the average temperature as a predictor (`r signif(m_precise_n16wk$aic, digits = 3)` compared to `r signif(m_precise_avtemp7$aic, digits = 3)`).  

The FMP database contains data on the severity of outbreaks (the estimated proportion of the frog population that died) for the years 1991-2000. After removing records with missing values, we produced a dataset for investigating severity that contained `r nrow(dsev_fmp)` records, of which `r dsev_fmp %>% filter(rv_signs_bin == "yes") %>% nrow()` incidents were classified as ranavirosis-consistent. In a simple logistic model of severity with ranavirus status, average daily maximum temperature for the month of onset of mortality, and their interaction as predictors, all three terms were significant predictors and retained in the minimal adequate model. Temperature explained the most deviance with each 1°C increase in temperature leading to a `r signif(100*(as.vector(exp(m_sev_rvsigns_temp$coefficients[3]))-1), digits = 2)`% increase in the proportion of the population that died (p = $`r signif(summary(m_sev_rvsigns_temp)$coefficients[3,4], digits = 3)`$). There was a significant interaction between the two main effects as a consequence of the different effect of ranavirus status on the relationship between temperature and the proportion dead (p = `r signif(summary(m_sev_rvsigns_temp)$coefficients[4,4], digits = 1)`): at low temperatures, the severity of ranavirosis-consistent incidents was slightly lower than for other types of incident but at higher temperatures it was the ranavirosis-consistent incidents that were more severe (Figure 1d; Supporting information Figure S4). Attempting to incorporate time (the year that mortality incidents began) did not result in an extension of the minimal adequate model.  
...
```{r}
source("R/hist_cc_seasonality.R")
source("R/fig2.R")
```

Temperatures in our study region varied across years, showing a warming trend that peaked in 2002 before cooling up to 2010, as well as showing marked seasonality (GAM component of GAMM: R^2^ = `r round(summary(m_temp_decomp$gam)$r.sq, 2)`; Table S3). The proportion of ranavirosis incidents followed a remarkably similar pattern, in terms of the strong seasonality and the trend across years (GAM component of GAMM: R^2^ = `r round(summary(m_rv_decomp$gam)$r.sq, 2)`; Table S4; Figure 2a). The across-year temperature trend was a highly significant predictor of the rate of ranavirus incidents (GAM: effective degrees of freedom (edf) of smoothed term = `r signif(summary(m_gam_bin_wTime)$s.table[2,1], 3)`, p = $`r signif(summary(m_gam_bin_wTime)$s.table[2,4], 3)`$), explaining the pattern of increasing rates which peaked in 2001, with the full model explaining approximately half the deviance (R^2^ = `r round(summary(m_gam_bin_wTime)$r.sq, 2)`; Figure 2; Table S5). Model validation indicated conformity to assumptions. The temperature model, when trained on subsets of the data, predicted trends in test datasets effectively in contrast to models incorporating only the across-year trend in ranavirosis rates, which showed no predictive power (Supporting information Figure S6). Temperature showed a strong seasonal pattern which was reflected in the pattern of ranavirosis incidents where seasonality was also marked (GAM: edf = `r signif(summary(m_gam_bin_wTime)$s.table[1,1], 3)`, p = $`r signif(summary(m_gam_bin_wTime)$s.table[1,4], 3)`$; Figure 2c), raising the possibility that another seasonal factor - correlated with temperature - could have driven ranavirus outbreaks. However, the association between temperature and the rate of ranavirus incidents across years suggests that temperature is driving the rate of disease incidents in the long term and may also drive the seasonality, since both a correlated seasonal factor and a correlated non-seasonal factor acting across years would otherwise be required to explain the observed patterns. Overall, after accounting for the effect of temperature, there was a small but significant decrease in the proportion of ranavirosis incidents between 1991 and 2010 (GAM: coefficient = `r signif(summary(m_gam_bin_wTime)$p.table[2,1], 3)` [unit of time was months], p = $`r signif(summary(m_gam_bin_wTime)$p.table[2,4], 3)`$; Figure 2d).  
...
We examined *in vitro* viral growth using two UK isolates of FV3 (RUK11 and RUK13; see methods for detailed descriptions of isolates) and two cell lines. Each isolate was incubated at a range of temperatures up to 30°C with each cell line, but regardless of cell line or isolate, increasing temperature resulted in exponentially increased rates of plaque formation (Figure 3). A linear model of log viral titers against temperature, cell line and their interaction revealed significant effects of temperature (coefficient = `r round(coef(m_invitro)[2], digits = 2)`, p = `r round(summary(m_invitro)$coefficients[2,4], digits = 2)`) and host cell line (IgH2 compared to EPC, coef = `r round(coef(m_invitro)[3], digits = 2)`, p = `r round(summary(m_invitro)$coefficients[3,4], digits = 2)`) but no interaction (analysis of variance, comparing model with interaction term to a model with main effects only: F~df\ =\ `r anova(m_invitro, m_invitro2)$Df[2]`~ = `r signif(anova(m_invitro, m_invitro2)$F[2], digits = 2)`, p = `r signif(anova(m_invitro, m_invitro2)$Pr[2], digits = 2)`), indicating the overall effect of temperature was independent of the host environment.  

```{r Fig3}
# greyscale plot
ggplot(
  grid_invitro, 
  aes(x = temp, y = pred, group = cell)
  ) +
  geom_line(aes(linetype = cell), size = .7) +
  scale_linetype_manual(values = c("solid", "twodash")) +
  geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 0.3) +
  geom_point(data = d_fv3_invitro,
             aes(x = temp,
                 y = ltcid,
                 shape = cell),
             size = 2.5,
...
ggsurvplot(
  ...
  font.x = c(15, "plain", "#000000"),
  font.y = c(15, "plain", "#000000"),
  font.tickslab = c(12, "plain", "#000000"),
  # legend:
  legend = "bottom",
  legend.title = "Treatment",
  legend.labs = c(
    "Sham/20°C", "Sham/27°C", 
    "RUK13/20°C", "RUK13/27°C", 
    "RUK11/20°C", "RUK11/27°C"
  ),
  font.legend = c(12, "plain", "#000000")
)
...
```{r FigS1}
include_graphics("figs/fdose1_wPicsSig.png")
```
...
```{r FigS2}
# Violin plot: Box temperatures
dbox_temp %>% 
  ggplot(aes(x = box, y = temp, fill = box)) +
  geom_ref_line(h = 27, colour = "grey80", size = 1.2) +
  geom_ref_line(h = 20, colour = "grey80", size = 1.2) +
  geom_violin(lwd=.8) +
  # plot theme:
  theme_bw() +
  theme( panel.grid.major = element_blank(),
  ...
  scale_fill_manual(values = rep("#888888", 4)) + #
  # axes
  theme(axis.ticks = element_line(colour = 'black', size = .7),
...
```{r FigS4}
# severity of ranavirus vs other causes
dsev_fmp %>% 
  ggplot(
    aes(x = factor(rv_signs_bin, labels = c("Negative", "Positive")), 
        y = severity_hf, 
  ...
  theme(legend.position="right",
        ...
        legend.title = element_text(colour = "black", size = 13, face = "plain"),
        legend.key.size = unit(1, "cm")
        )
```
...
```{r FigS7}
# Effect of dose on timing of disease
# Timing of signs by dose stats
sim_pois_dpe <- tibble(
  eventd = rpois(length(dtiming$dpe[is.na(dtiming$dpe) == FALSE]),
                 mean(dtiming$dpe, na.rm = TRUE))
  )
counts_sim_pois_dpe <- sim_pois_dpe %>%
  dplyr::count(eventd, name = "count")

# dtiming %>%
#   ggplot(aes(x = dpe)) +
#   geom_histogram(binwidth = 1) +
#   geom_line(aes(x = eventd, y = count), data = counts_sim_pois_dpe)

# GLMM
mtiming_me <- glmer(dpe ~ dose + sign + (1 | id), family = poisson, data = dtiming)
# mtiming_me <- glmm(dpe ~ dose + sign, list(dpe ~ 0 + id), family.glmm = poisson.glmm, data = dtiming, varcomps.names = c("id"), m = 1000)
# summary(mtiming_me)

dtiming %>%
  filter(sign != 'eye') %>%
  ggplot(aes(x = sign, y = dpe, fill = dose)) +
  geom_boxplot() +
  scale_x_discrete(
    limits = c("ulcer", "petechiae_ext", "vomit", "bleeding", "any_red", "stop_eat"),
    labels = c("Ulceration", "Petechiae", "Vomiting", "Bleeding", "Reddening", "Inappetence")
    ) +
  facet_grid(dose ~ .) +
  scale_fill_manual(values = c(cTourq, cOrange)) +
  labs(y = "Number of days post-exposure", x = "Sign of disease") +
  coord_flip() +
  # plot theme:
  theme_bw() +
  theme( panel.grid.major = element_blank(),
...
```{r Table S1: incidence coefficients, echo=FALSE}
tabS1 <- cbind(term = rownames(summary(m_incidence_mam1)$coefficients),
               as_tibble(summary(m_incidence_mam1)$coefficients))
knitr::kable(tabS1, caption = "Table S1. Model output from logistic model of incidence of ranavirus outbreaks as function of a range of variables describing the local environment of amphibian mortality incidents from Frog Mortality Project reports.", digits=3)
```

```{r Table S2: severity coefficients, echo=FALSE}
tabS2 <- cbind(term = rownames(summary(m_sev_rvtemp_full_mam)$coefficients),
               as_tibble(summary(m_sev_rvtemp_full_mam)$coefficients))
knitr::kable(tabS2, caption = "Table S2. Model output from linear model of outbreak severity as function of a range of variables describing the local environment of amphibian mortality incidents from Frog Mortality Project reports.", digits=3)
```
...
```{r Table S3: temp GAMM - GAM component, echo=FALSE}
tabS3a <- cbind(term = rownames(summary(m_temp_decomp$gam)$p.table),
               as_tibble(summary(m_temp_decomp$gam)$p.table))
tabS3b <- cbind(term = rownames(summary(m_temp_decomp$gam)$s.table),
                as_tibble(summary(m_temp_decomp$gam)$s.table))

knitr::kable(tabS3a, caption = "a) parametric component", digits=3)
knitr::kable(tabS3b, caption = "b) Smoothed component", digits=3)
```
...
```{r Table S4: ranavirus GAMM - GAM component, echo=FALSE}
tabS4a <- cbind(term = rownames(summary(m_rv_decomp$gam)$p.table),
               as_tibble(summary(m_rv_decomp$gam)$p.table))
tabS4b <- cbind(term = rownames(summary(m_rv_decomp$gam)$s.table),
                as_tibble(summary(m_rv_decomp$gam)$s.table))

knitr::kable(tabS4a, caption = "a) parametric component", digits=3)
knitr::kable(tabS4b, caption = "b) Smoothed component", digits=3)
```
...
```{r Table S5: GAM, echo=FALSE}
tabS5a <- cbind(term = rownames(summary(m_gam_bin_wTime)$p.table),
               as_tibble(summary(m_gam_bin_wTime)$p.table))
tabS5b <- cbind(term = rownames(summary(m_gam_bin_wTime)$s.table),
                as_tibble(summary(m_gam_bin_wTime)$s.table))

knitr::kable(tabS5a, caption = "a) parametric component", digits=3)
knitr::kable(tabS5b, caption = "b) Smoothed component", digits=3)
```
